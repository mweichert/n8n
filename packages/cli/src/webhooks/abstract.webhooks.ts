...
export abstract class AbstractWebhooks {
	...
	abstract executeWebhook(req: WebhookRequest, res: Response): Promise<WebhookResponseCallbackData>;

	constructor(protected readonly nodeTypes: INodeTypes) {}

	registerHandler(app: Application, route: string) {
		app.all(route, async (req: WebhookRequest | WebhookCORSRequest, res) =>
	...
	private preWebhookExecution(startNode: INode) {
	...
	private async handleRequest(req: WebhookRequest | WebhookCORSRequest, res: Response) {
		...
		if ('origin' in req.headers) {
			try {
				const allowedMethods = await this.getWebhookMethods(path);
		...
		let response;
		try {
			response = await this.executeWebhook(req, res);
		} catch (error) {
			return sendErrorResponse(res, error as Error);
		}
